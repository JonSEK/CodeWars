...
Friends inhabit towns and you get an array with friends and their towns (or an associative array), for example: [["A1", "X1"], ["A2", "X2"], ["A3", "X3"], ["A4", "X4"]] which means A1 is in town X1, A2 in town X2... 
It can happen that we do not know the town of one of the friends hence it will not be visited.
Your granny wants to visit her friends and to know approximately how many miles she will have to travel. 
You will make the circuit that permits her to visit her friends. For example here the circuit will be:X0, X1, X2, X3, X4, X0 and you will compute approximately the total distance X0X1 + X1X2 + .. + X4X0.
...
function tour(friends, fTowns, distTable) {
  ...
  const townDistMap = fTowns.reduce((map, [friend, town]) => {
    const distIndex = distTable.indexOf(town);
    if (distIndex !== -1) {
      map[friend] = distTable[distIndex + 1];
    }
    return map;
  }, {});
  ...
  for (let i = 0; i < friends.length; i++) {
    ...
    if (townDistMap.hasOwnProperty(friend)) {
      preDist = curDist;
      curDist = townDistMap[friend];
      if (i !== 0) {
        result += Math.sqrt(Math.pow(curDist, 2) - Math.pow(preDist, 2));
      } else {
        result += curDist;
      }
    }
